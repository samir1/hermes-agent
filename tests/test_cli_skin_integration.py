...
from hermes_agent.cli.repl import HermesCLI, _build_compact_banner, _rich_text_from_ansi
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_hermes_branding(self):
        ...
        with patch("hermes_agent.cli.repl.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("hermes_agent.cli.repl.format_banner_version_label", return_value="Hermes Agent v0.1.0 (test)"):
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("hermes_agent.cli.repl.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("hermes_agent.cli.repl.format_banner_version_label", return_value="Hermes Agent v1.0 (test) · upstream abc12345"):
            banner = _build_compact_banner()
