...
from hermes_agent.cli.setup_wizard import _print_setup_summary, setup_model_provider
...
def test_setup_same_provider_rotation_strategy_saved_for_multi_credential_pool(tmp_path, monkeypatch):
    ...
    import hermes_agent.cli.setup_wizard as _setup_mod
    import hermes_agent.providers.credential_pool as _pool_mod
    import hermes_agent.providers.auxiliary as _aux_mod
...
def test_setup_same_provider_fallback_can_add_another_credential(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("hermes_agent.providers.credential_pool.load_pool", fake_load_pool)
    monkeypatch.setattr("hermes_agent.cli.auth.commands.auth_add_command", fake_auth_add_command)
    monkeypatch.setattr("hermes_agent.providers.auxiliary.get_available_vision_backends", lambda: [])
...
def test_setup_same_provider_single_credential_keeps_existing_rotation_strategy(tmp_path, monkeypatch):
    ...
    _stub_tts(monkeypatch)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("hermes_agent.providers.credential_pool.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("hermes_agent.providers.auxiliary.get_available_vision_backends", lambda: [])
...
def test_setup_pool_step_shows_manual_vs_auto_detected_counts(tmp_path, monkeypatch, capsys):
    ...
    _stub_tts(monkeypatch)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("hermes_agent.providers.credential_pool.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("hermes_agent.providers.auxiliary.get_available_vision_backends", lambda: [])
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("hermes_agent.cli.auth.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("hermes_agent.providers.auxiliary.get_available_vision_backends", lambda: [])
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_agent.providers.auxiliary.get_available_vision_backends", lambda: ["anthropic"])
