...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.cli.config.get_hermes_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "hermes_agent.cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "hermes_agent.cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpRemove:
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "hermes_agent.cli.mcp_config.get_hermes_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "hermes_agent.cli.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "hermes_agent.cli.mcp_config._MCP_PRESETS",
