...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.auth.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("hermes_agent.cli.auth.copilot._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        ...
        monkeypatch.setattr("hermes_agent.cli.auth.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("hermes_agent.cli.auth.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("hermes_agent.cli.auth.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("hermes_agent.cli.auth.copilot._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("hermes_agent.cli.auth.copilot.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "hermes_agent.cli.auth.copilot.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "hermes_agent.cli.auth.copilot.os.access",
        ...
        monkeypatch.setattr("hermes_agent.cli.auth.copilot.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("hermes_agent.cli.auth.copilot._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "hermes_agent.cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "hermes_agent.cli.models.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: hermes_home / ".env")
        monkeypatch.setattr(config_module, "get_hermes_home", lambda: hermes_home)
        monkeypatch.setattr("hermes_agent.cli.auth.copilot.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("hermes_agent.cli.auth.auth.get_auth_status", lambda _pid: {})
        # Simulate valid Claude Code credentials
        monkeypatch.setattr(
            "hermes_agent.providers.anthropic_adapter.read_claude_code_credentials",
            lambda: {"accessToken": "sk-ant-test", "refreshToken": "ref-tok"},
        )
        monkeypatch.setattr(
            "hermes_agent.providers.anthropic_adapter.is_claude_code_token_valid",
            lambda creds: True,
        )
        from hermes_agent.cli.main import _has_any_provider_configured
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "hermes_agent.cli.auth.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("hermes_agent.cli.auth.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from hermes_agent.cli.models.models import _PROVIDER_MODELS as models_models
    ...
    def test_model_metadata_has_context_lengths(self):
        ...
        from hermes_agent.cli.models.models import _PROVIDER_MODELS
        from hermes_agent.providers.metadata import DEFAULT_CONTEXT_LENGTHS
    ...
    def test_provider_aliases_in_models_py(self):
        from hermes_agent.cli.models.models import _PROVIDER_ALIASES
        assert _PROVIDER_ALIASES.get("hf") == "huggingface"
        assert _PROVIDER_ALIASES.get("hugging-face") == "huggingface"

    def test_provider_label(self):
        from hermes_agent.cli.models.models import _PROVIDER_LABELS
