...
from hermes_agent.cli.auth.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from hermes_agent.cli.auth.auth import _resolve_verify
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    from hermes_agent.providers.credential_pool import PooledCredential, load_pool
    pool = load_pool("nous")
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.auth.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("hermes_agent.cli.auth.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import hermes_agent.cli.models.models as models_mod
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from hermes_agent.cli.auth.auth import PROVIDER_REGISTRY, _login_nous
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    """
    from hermes_agent.cli.auth.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    """
    from hermes_agent.cli.auth.auth import persist_nous_credentials, resolve_nous_runtime_credentials
