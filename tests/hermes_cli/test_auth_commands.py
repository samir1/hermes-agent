...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.providers.anthropic_adapter.run_hermes_oauth_login_pure",
        lambda: {
            "access_token": token,
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.cli.auth.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.cli.auth.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr(
        "hermes_agent.providers.credential_pool._seed_from_singletons",
    ...
    from hermes_agent.cli.auth.commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from hermes_agent.cli.auth.commands import auth_reset_command
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from hermes_agent.cli.auth.commands import auth_list_command
    ...
    monkeypatch.setattr(
        "hermes_agent.cli.auth.commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_agent.cli.auth.commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("hermes_agent.cli.auth.commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_agent.cli.auth.commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "hermes_agent.cli.auth.commands.time.time",
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from hermes_agent.cli.auth.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from hermes_agent.cli.auth.auth import unsuppress_credential_source
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.auth.auth._import_codex_cli_tokens", _fake_import)

    from hermes_agent.providers.credential_pool import _seed_from_singletons
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from hermes_agent.cli.auth.auth import is_source_suppressed
    from hermes_agent.cli.auth.commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    from hermes_agent.providers.credential_pool import _seed_from_env
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import hermes_agent.cli.auth.copilot as ca
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import hermes_agent.cli.auth.auth as ha
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    ...
    import hermes_agent.providers.anthropic_adapter as aa
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    from hermes_agent.providers.credential_pool import _seed_custom_pool, get_custom_provider_pool_key
...
def test_credential_sources_registry_has_expected_steps():
    ...
    """
    from hermes_agent.providers.credential_sources import _REGISTRY
