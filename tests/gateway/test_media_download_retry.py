...
class TestSSRFRedirectGuard:
    ...
    def test_image_blocks_private_redirect(self, tmp_path, monkeypatch):
        ...
        async def run():
            with patch("hermes_agent.tools.security.urls.is_safe_url", side_effect=fake_safe), \
    ...
    def test_safe_redirect_allowed(self, tmp_path, monkeypatch):
        ...
        async def run():
            with patch("hermes_agent.tools.security.urls.is_safe_url", return_value=True), \
                 patch("httpx.AsyncClient", side_effect=factory):
                from hermes_agent.gateway.platforms.base import cache_image_from_url
                return await cache_image_from_url(
