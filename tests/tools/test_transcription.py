...
class TestTranscribeAudio:
    ...
    def test_dispatches_to_local(self, tmp_path):
        ...
        with patch("hermes_agent.tools.media.transcription._load_stt_config", return_value={"provider": "local"}), \
             patch("hermes_agent.tools.media.transcription._get_provider", return_value="local"), \
             patch("hermes_agent.tools.media.transcription._transcribe_local", return_value={"success": True, "transcript": "hi"}) as mock_local:
    ...
    def test_dispatches_to_openai(self, tmp_path):
        ...
        with patch("hermes_agent.tools.media.transcription._load_stt_config", return_value={"provider": "openai"}), \
             ...
             patch("hermes_agent.tools.media.transcription._transcribe_openai", return_value={"success": True, "transcript": "hi"}) as mock_openai:
            from hermes_agent.tools.media.transcription import transcribe_audio
            result = transcribe_audio(str(audio_file))
    ...
    def test_disabled_config_returns_disabled_error(self, tmp_path):
        ...
        with patch("hermes_agent.tools.media.transcription._load_stt_config", return_value={"enabled": False}), \
             patch("hermes_agent.tools.media.transcription._get_provider", return_value="none"):
            from hermes_agent.tools.media.transcription import transcribe_audio
            result = transcribe_audio(str(audio_file))
...
class TestNormalizeLocalModel:
    ...
    def test_openai_model_name_maps_to_default(self):
        from hermes_agent.tools.media.transcription import _normalize_local_model, DEFAULT_LOCAL_MODEL
    ...
    def test_valid_local_model_preserved(self):
        from hermes_agent.tools.media.transcription import _normalize_local_model
    ...
    def test_warning_emitted_for_cloud_model(self, caplog):
        import logging
        from hermes_agent.tools.media.transcription import _normalize_local_model
        with caplog.at_level(logging.WARNING, logger="hermes_agent.tools.media.transcription"):
    ...
    def test_local_transcribe_normalises_model(self):
        ...
        try:
            ...
            with patch("hermes_agent.tools.media.transcription._HAS_FASTER_WHISPER", True), \
                 patch("hermes_agent.tools.media.transcription._load_stt_config", return_value={
                     "enabled": True,
                     "provider": "local",
