...
- **Default 4 parallel tasks**: batches default to 4 concurrent subagents (configurable via `delegation.max_concurrent_children` in config.yaml, no hard ceiling, only a floor of 1)
- **Nested delegation is available by default**: leaf subagents (default) cannot call `delegate_task`, `clarify`, `memory`, `send_message`, or `execute_code`. Orchestrator subagents (`role="orchestrator"`) retain `delegate_task` for further delegation when `delegation.max_spawn_depth >= 2` (1-3 supported, default 2); the other four remain blocked. Disable globally via `delegation.orchestrator_enabled: false`.

### Tuning Concurrency and Depth

| Config | Default | Range | Effect |
|--------|---------|-------|--------|
| `max_concurrent_children` | 4 | >=1 | Parallel batch size per `delegate_task` call |
| `max_spawn_depth` | 2 | 1-3 | How many delegation levels can spawn further |
